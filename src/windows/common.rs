...
/// Flag indicating the event was injected (programmatically generated) for keyboard hooks
pub const LLKHF_INJECTED: DWORD = 0x00000010;
/// Flag indicating the event was injected (programmatically generated) for mouse hooks
pub const LLMHF_INJECTED: DWORD = 0x00000001;

/// Check if a keyboard event is injected (synthetic/programmatic)
pub unsafe fn is_keyboard_injected(lpdata: LPARAM) -> bool {
    unsafe {
...
/// Check if a mouse event is injected (synthetic/programmatic)
...
/// Standard wheel delta unit (120 = one "notch")
...
pub fn set_modifier(key: Key, down: bool) {
    KEYBOARD.lock().set_modifier(key, down);
}

pub fn get_modifier(key: Key) -> bool {
    KEYBOARD.lock().get_modifier(key)
}
